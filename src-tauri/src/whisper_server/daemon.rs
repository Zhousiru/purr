...
pub fn spawn_daemon(program: String, args: Vec<String>, app: AppHandle) -> Daemon {
  ...
  thread::spawn(move || {
    app
      .emit_all("whisper-server-daemon", WhisperServerEvent::Launch)
      .unwrap();
    ...
    thread::spawn(move || {
      ...
      app
        .emit_all("whisper-server-daemon", WhisperServerEvent::Exit)
        .unwrap();
